...
roughDataDir <- "./rawData"
roughDataUrl <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
roughDataFilename <- "rawData.zip"
roughDataDFn <- paste(roughDataDir, "/", "rawData.zip", sep = "")
dataDir <- "./data"
if (!file.exists(roughDataDir)) {
  dir.create(roughDataDir)
  download.file(url = roughDataUrl, destfile = roughDataDFn)
}

if (!file.exists(dataDir)) {
  dir.create(dataDir)
  unzip(zipfile = roughDataDFn, exdir = dataDir)
}
